...
import { X, Plus, Minus, Save, Calendar, Clock, User, Phone, ChevronDown, ChevronRight } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
import { mn } from '../../utils/mongolian';
import { Service, Employee, Appointment, TransactionService, Commission } from '../../types';
import { getServices, getEmployees, saveAppointment, deleteRemindersByAppointmentId, createAppointmentReminder } from '../../utils/storage';
...
const AppointmentEditForm: React.FC<AppointmentEditFormProps> = ({ appointment, onClose, onSave }) => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({
    ...
    selectedServices: appointment.services.map(s => ({ ...s, quantity: 1 })),
    selectedEmployees: appointment.employees,
  ...
  const toggleEmployee = (employeeId: string) => {
    ...
    if (isSelected) {
      ...
      // Remove the limit - allow unlimited employees
      setFormData({
        ...formData,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Delete old reminders and create new ones if date/time changed
    ...
    const newDateTime = new Date(`${formData.appointmentDate}T${formData.appointmentTime}`);
    
    if (originalDateTime.getTime() !== newDateTime.getTime()) {
      // Delete old reminders
      deleteRemindersByAppointmentId(appointment.id);
      
      // Create new reminder
      createAppointmentReminder(updatedAppointment);
    }
      ...
      <div className="bg-gray-900 w-full max-h-[90vh] rounded-t-3xl overflow-hidden">
        <div className="sticky top-0 bg-gray-900 p-6 border-b border-gray-800">
          <div className="flex items-center justify-between">
            <h2 className="text-white text-xl font-bold">Цаг захиалга засах</h2>
            <button
              onClick={onClose}
